   ...
   return (
      <Box sx={{ width: '100%', height: '80px', display: 'flex', flexDirection: 'row', 
         ...
         <Link href='/' sx={{ textDecoration: 'none' }}>
            ...
            <Box sx={{ color: 'var(--color-LGgray)', fontSize: '20px', fontWeight: '800', mt: '-15px', pl: '60px' }}>
               Japan Lab Inc.</Box>
         </Link>
               ...
               <li key={idx} className={styles.link}
                  ...
                  <Link href={link.href} sx={{ height: '100%', lineHeight: '80px' }}>
                     <Typography sx={{ height: '100%', lineHeight: '80px', px: '10px', display: 'flex', 
                                       alignItems: 'center', '&:hover': { color: 'var(--color-LGred)' } }}>
                        {link.title}
                        {link.subLinks && 
                     ...
                     {link.subLinks && link.subLinks.map((subLink, idx) => (
                        <li key={idx}>
                           <Link href={subLink.href}>
                              <Typography sx={{ '&:hover': { color: 'var(--color-LGred)', } }}>{subLink.title}</Typography>
                           </Link>
                        </li>
                     ))}
                  </ul>
