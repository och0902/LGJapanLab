...
import ModalPledge from '@/components/en/ModalPledge/ModalPledge';
...
const VisitApply = () => {
	...
	const handleSubmit = async (e) => {
		...
		if ( !visitInfo.company || !visitInfo.name || !visitInfo.email || !visitInfo.mobile || !visitInfo.visitDate || !visitInfo.visitTime ) 
			{ toast.error('Please enter all required items.'); return; };
		if ( !visitInfo.email.includes('@lg') ) 
			{ toast.error('Only email addresses of LG Group affiliates are accepted. '); return;	};
		if ( visitInfo.privacyCheck !== true || visitInfo.pledgeCheck !== true ) 
			{ toast.error('Please agree to the privacy policy and visitor pledge.'); return; }
			...
			<form style={{ width: '100%' }} onSubmit={handleSubmit}>
				...
				<Box sx={{ width: '80%', m: 'auto', [theme.breakpoints.down('lg')] : { width: '100%' } }}>
					<Box sx={{ m: '50px 20px 30px 20px', fontSize: '1.3rem' }}>
						<CircleIcon sx={{ fontSize: '0.6rem', color: 'var(--color-LGred)' }} /> Visitor Information
					</Box>
					<Grid container >
